...
class Graph{
    ...
    void input(bool direct,bool weight){
        ...
        for(int i=0;i<Edge_num;i++){
            if(weight)cin>>from>>to>>cost;
            else {cin>>from>>to;cost=1;}
    ...
    vector<bool>visited=new vector<bool>(Vertex_num);//dfs用
...
//     // bool visited[v];//dfsで使用
    ...
    vector<T>*depth=new vector<T>(Vertex_num,0),*parent=new vector<T>(Vertex_num,-1);
    vector<T> Tree_dfs(int current_v,int parent_v,T cost){
        parent[current_v]=parent_v;
        depth[current_v]=cost;
        for(auto x:g[current_v])if(x.to!=parent_v)dfs(x.to,current_v,cost+x.cost);//(木の子孫は全て未訪問)
    }
    void tree_dfs(int root){
        Tree_dfs(root,-1,0LL);//再帰関数の呼び出し
    }

    T tree_diameter(){
        vector<T>a=this->bfs(0);
        int idx=-1,ma=-1;
        for(int i=0;i<Vertex_num;i++)if(ma<a[i])ma=a[i],idx=i;
        a=this->bfs(idx);
        ma=-1;
        for(int i=0;i<Vertex_num;i++)if(ma<a[i])ma=a[i];
        return ma;
    }
...
int main(){
    cin>>V>>E;
    Graph<long long>graph(V,E);
    graph.flow_input();
    //graph.ford_fulkerson(0,V-1);
    return 0;
}
